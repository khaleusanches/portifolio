...
- [x] ~~Sob `prefers-reduced-motion: reduce` as três peças mostram o estado final sem
      transição~~ — revertido a pedido da Brand: nenhuma animação consulta a
      preferência. Ver spec 0003, "O movimento não se reduz".
- [x] ~~Sob `prefers-reduced-motion: reduce` a vitrine não rola sozinha~~ — revertido
      junto com o item acima.
- [x] ~~A preferência é respeitada se mudar durante a visita, sem recarregar a página~~
      — sem efeito: a preferência não é consultada.
